...
class DAGScheduler(
  ...
  //todo:需要运行的stage，但是得等这些stage的父stage运行完才行
  ...
  //todo：创建了DAGSchedulerEventProcessLoop，创建之后，需要看DAGSchedulerEventProcessLoop的构造和父类的构造
      ...
      case None =>
        ...
        val stage =
          ...
          newOrUsedStage(
            //todo：拿出这个RDD
            shuffleDep.rdd,
            shuffleDep.rdd.partitions.size, shuffleDep, jobId,
  ...
  private def getParentStages(rdd: RDD[_], jobId: Int): List[Stage] = {
    ...
    val waitingForVisit = new Stack[RDD[_]]//todo:等待被访问的RDD栈
    //todo：定义方法，还没调用
    def visit(r: RDD[_]) {
      if (!visited(r)) {//todo：RDD不在栈中，进入逻辑
        visited += r//todo：把RDD放到set中
  ...
  def submitJob[T, U](
    ...
    //todo:将JobSubmitted提交到任务队列eventProcessLoop中
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    if (finalStage != null) {
      ...
      if (shouldRunLocally) {//todo:local模式
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing == Nil) {//todo：没有父stage，递归出口
          logInfo("Submitting " + stage + " (" + stage.rdd + "), which has no missing parents")
          //todo:提交taskset到executor
          submitMissingTasks(stage, jobId.get)
        } else {//todo:有父stage，继续递归
          for (parent <- missing) {
            submitStage(parent)
          }
          waitingStages += stage//todo：将stage放到等待队列中
        }
      }
    } else {
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    val tasks: Seq[Task[_]] = if (stage.isShuffleMap) {
      //todo:根据分区的数量n，封装相同的n个task到taskset，下发到每一个分区
    ...
    if (tasks.size > 0) {
      ...
      //todo:将stage封装成taskSet，提交taskSet
