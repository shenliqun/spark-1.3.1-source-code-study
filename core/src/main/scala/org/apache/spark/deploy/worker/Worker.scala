...
private[spark] class Worker(
  ...
  val executors = new HashMap[String, ExecutorRunner] //todo：创建了封装executor的集合
  val finishedExecutors = new HashMap[String, ExecutorRunner] //todo：创建了封装executor的集合（完成的executor）
  ...
  def registerWithMaster() {
    ...
    registrationRetryTimer match {
      case None =>
        ...
        registrationRetryTimer = Some {
          //todo：定义调度器，尅是10秒之后执行，间隔10秒（心跳）
          conctext.system.scheduler.shedule(INITIAL_REGISTRATION_RETRY_INTERVAL,
    ...
    case LaunchExecutor(masterUrl, appId, execId, appDesc, cores_, memory_) =>
      if (masterUrl != activeMasterUrl) {
        ...
        try {
          ...
          //todo:创建ExecutorRunner，封装参数，启动executor
