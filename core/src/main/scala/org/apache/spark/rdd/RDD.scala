...
abstract class RDD[T: ClassTag](
  ...
  /**
   * Internal（内部的） method to this RDD; will read from cache if applicable（可用的）, or otherwise（否则） compute it.
   * This should ''not'' be called by users directly（直接的）, but is available（有效的） for implementors of custom
   * subclasses of RDD.
   */
  ...
  /**
   * Return the ancestors（祖先） of the given RDD that are related（有关联的） to it only through a sequence of
   * narrow dependencies. This traverses（横向运动） the given RDD's dependency tree using DFS, but maintains
   * no ordering on the RDDs returned.
   */
  private[spark] def getNarrowAncestors: Seq[RDD[_]] = {
    val ancestors = new mutable.HashSet[RDD[_]]
    //todo：通过visit方法，找到：上一个宽依赖--》当前rdd  之间的所有窄依赖，不包含自身
    def visit(rdd: RDD[_]) {
