...
/**
  *   spark提交任务的入口
  *   主构造器中：
  *   1、调用createSparkEnv方法创建sparkEnv，用于创建actorSystem
  *   2、创建taskScheduler
  *   3、创建DAGScheduler
...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  //TODO:创建sparkenv，用于创建actorsystem
  ...
  def runJob[T, U: ClassTag](
    ...
    //todo：DAGscheduler在sparkcontent的主构造器中创建的
    //todo：现在开始切分stage，查分pipeline，提交taskset提交给taskScheduler
  ...
  def runJob[T, U: ClassTag](rdd: RDD[T], func: (TaskContext, Iterator[T]) => U): Array[U] = {
    //todo：rdd：最后一个rdd；func：writeToFile这个函数；0 until rdd.partitions.size：分区数；false：是否在本地执行
  ...
  private def createTaskScheduler(
    ...
    master match {
      ...
      case SPARK_REGEX(sparkUrl) =>
        ...
        //TODO：SparkDeploySchedulerBackend
        val backend = new SparkDeploySchedulerBackend(scheduler, sc, masterUrls)
        //TODO:将SparkDeploySchedulerBackend放到TaskSchedulerImpl的成员变量，以备后期使用
        scheduler.initialize(backend)
        //TODO：返回
        (backend, scheduler)
